...
def getRareData(cType,nID):
    ...
    printString = str(counterProg[cType])+" / "+rareAmounts[cType]+"\t\t"+str(nID)

    #build the final rarespawn id url
    url = mapUrl[cType] + str(nID)
    #get webpage script data
    soup = BeautifulSoup(requests.get(url).text, "html.parser")

    rareSoupDict[cType][nID] = soup
...
def createThreads(coreType):
    for npcId in rares[coreType]:
        t = threading.Thread(target=getRareData,args=[coreType,npcId])
        t.start()
        threads.append(t)
...
with open("rareSoupDictAC.pkl", "wb") as af:
    pickle.dump(rareSoupDict["AC"], af)
with open("rareSoupDictTC.pkl", "wb") as tf:
    pickle.dump(rareSoupDict["TC"], tf)